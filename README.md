...
```
RECON-PLAN (LLM 动态决定侦察维度)
   ↓
RECON (每维度一个隔离 subagent, 返回结构化 dossier)
   ↓  recon_score (置信度路由: solid / revisit / escalate)
PLAN (TwoRoundGate: R1 起草 → R2 对抗审查)
   ↓  init yaml + slices
SLICE EXECUTE (拓扑序, 每 slice: Design → Implement → Verify)
   ↓  check_slice_gate → done / blocked / verifying
FINAL (build_standard + final_check → DONE / PAUSED)
```
