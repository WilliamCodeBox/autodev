...
import { parse as yamlParse, stringify as yamlStringify } from './yaml-lite.mjs';

export const TASK_STATUS = ['todo', 'doing', 'blocked', 'done'];
export const SLICE_STAGES = ['queued', 'planning', 'executing', 'verifying', 'done', 'blocked', 'paused'];
...
export function saveAutodev(root = '.', doc) {
  ...
  fs.writeFileSync(p, yamlStringify(doc));
  return p;
}
...
export function saveSlice(root = '.', slice) {
  ...
  fs.mkdirSync(path.dirname(p), { recursive: true });
  fs.writeFileSync(p, yamlStringify(slice));
  return p;
}
